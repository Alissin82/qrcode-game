...
import MinuteCounter from '../components/ui/MinuteCounter.tsx';
...
const MissionDetailPage = () => {
    ...
    const handleDownload = () => {
        try {
            if (teamData?.gender)
                downloadFile(
                    action?.attachment_boy
                        .download_url!,
                    'file.pdf',
                );
            else
                downloadFile(
                    action?.attachment_girl
                        .download_url!,
                    'file.pdf',
                );
        } catch (error) {
    ...
    async function handleEnd(id: any) {
        if (id != action?.id) {
            toast.error(
                'لطفا کارت همین عملیات را اسکن کنید.',
            );
            return;
        }
                    ...
                    >
                        ...
                        >
                            {/* <div className='btn rounded-lg bg-purple-600 p-2'>
                                <img ref={''} alt={'action icon'} className={' w-[32px] h-[32px]'} />
                            </div> */}
