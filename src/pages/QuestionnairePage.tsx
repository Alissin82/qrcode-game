...
export const QuestionnairePage = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const res: AxiosResponse<ApiResponse<any>> = await apiClient.get(`/tasks/${id}`);
                setAnswers(Object.values(res.data.data.taskable.options));
            ...
            <div className="w-full max-w-xl p-4 text-center">
                ...
                <main className="space-y-6">
                    <div className="rounded-2xl bg-black/20 p-6">
                        <h2 className="text-2xl font-bold">سوالات چهار گزینه ای</h2>
                    </div>

                    <form onSubmit={handleSubmit} className="space-y-4 text-right">
                        <div className="my-2 rounded-2xl bg-black/20 p-6">
                            <h3 className="mb-4 font-bold">{taskData?.taskable.question}</h3>
                        </div>
